...
def get_movies():
    ...
    conn.request("GET", "/3/discover/movie?page=1&include_video=false&include_adult=false&sort_by=popularity.desc&language=en-US&api_key=776ecd6d5ef23c833c92236e4297d694", payload)
    ...
    for movie in results:
        ...
        conn.request("GET", "/3/movie/"+str(movie["id"])+"/videos?api_key=776ecd6d5ef23c833c92236e4297d694", payload)
        res = conn.getresponse()
        video_data = {}
        video_data = res.read()
        video_data_json = json.loads(video_data)
        # add the title/poster_path/video_url of each movie to latest_movies
        movie_info = [str(movie["title"]),"https://image.tmdb.org/t/p/w500"+str(movie["poster_path"]),"https://www.youtube.com/watch?v="+str(video_data_json["results"][0]["key"]),movie["overview"].encode('utf-8')]
